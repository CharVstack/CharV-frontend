...
import type { Methods as Methods3 } from './v1/vms/_vmId@string/power';
        ...
        _vmId: (val2: string) => {
          ...
          return {
            power: {
               ...
               */
              $post: (option: { body: Methods3['post']['reqBody']; config?: T | undefined }) =>
                fetch<void, BasicHeaders, Methods3['post']['status']>(prefix, `${prefix2}${PATH2}`, POST, option)
                  .send()
                  .then((r) => r.body),
              /**
               * VMの電源状態を取得
             ...
             */
            $delete: (option?: { config?: T | undefined } | undefined) =>
              fetch<void, BasicHeaders, Methods2['delete']['status']>(prefix, prefix2, DELETE, option)
                .send()
                .then((r) => r.body),
            $path: () => `${prefix}${prefix2}`,
          };
